...
def gather_results(startlist,column_names,table_size_dict):
    ...
    try:
        mock_stage_df = load_from_postgres('stages')
    ...
    try:
        mock_RIS_df = load_from_postgres('stage_results')    
    ...
    try:   
        mock_RIC_df = load_from_postgres('class_results')    
    ...
    for idx, rider_url in startlist['rider_url'].items():
        start_t2 = time.time()
        sums_and_averages,stage_df,RIS_df,RIC_df = get_latest_results(startlist,rider_url,stage_df,RIS_df,RIC_df,test=False)
